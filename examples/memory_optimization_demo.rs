...
use nyxs_owl::memory_optimized::{
    CacheOptimizedTimeSeries, 
    CacheOptimizedCircularBuffer, 
    MemoryPool, 
    CompactPrice
};
...
fn demo_cache_optimized_time_series() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ Retrieved last 100 prices and returns in: {:?}", tail_duration);
    println!("✓ Latest price: ${:.2}", last_100_prices[last_100_prices.len() - 1]);
    println!("✓ Latest return: {:.4}%", last_100_returns[last_100_returns.len() - 1] * 100.0);
...
fn demo_memory_pool_efficiency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for iteration in 0..1000 {
        ...
        for window in returns.windows(20) {
            ...
            let variance: f64 = window.iter().map(|r| (r - mean).powi(2)).sum::<f64>() / (window.len() - 1) as f64;
    ...
    println!("✓ Completed {} allocation cycles in: {:?}", total_allocations / 3, pool_duration);
    println!("✓ Final pool size: {} vectors available for reuse", memory_pool.pool_size());
    println!("✓ Average allocation time: {:?}", pool_duration / (total_allocations / 3));
    ...
    println!("✓ Standard allocation completed in: {:?}", standard_duration);
    
    let efficiency_improvement = (standard_duration.as_nanos() as f64 / pool_duration.as_nanos() as f64 - 1.0) * 100.0;
    if efficiency_improvement > 0.0 {
        println!("✓ Memory pool is {:.1}% more efficient!", efficiency_improvement);
    } else {
...
fn demo_compact_price_encoding() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compact_prices: Vec<CompactPrice> = sample_prices.iter()
    ...
    for (i, ((original_price, original_volume, original_timestamp), compact)) in 
        ...
        if price_error > 0.0001 || decoded_volume != *original_volume || decoded_timestamp != *original_timestamp {
            errors += 1;
            if errors <= 3 { // Show first few errors
                println!("  Error at {}: Price {:.6} -> {:.6} (error: {:.6})", 
    ...
    println!("✓ Total encoding errors: {} out of {}", errors, sample_prices.len());
...
fn demo_circular_buffer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..1000 {
        ...
        if rolling_buffer.len() >= 20 {
            ...
            if i % 100 == 0 {
                println!("  Step {}: Price = ${:.2}, 50-period avg = ${:.2}", 
                         i, price, avg);
            }
        }
    }
    ...
    println!("✓ Processed 1000 streaming updates in: {:?}", stream_duration);
    ...
    println!("✓ Last 10 values: {:?}", 
             recent_data.iter().map(|&&x| format!("{:.2}", x)).collect::<Vec<_>>());
...
fn demo_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Running performance comparison with {} data points...", data_size);
    ...
    let standard_duration = standard_start.elapsed();
    println!("   ✓ Standard approach completed in: {:?}", standard_duration);
    ...
    println!("   ✓ Optimized approach completed in: {:?}", optimized_duration);
    ...
    let memory_efficiency = (standard_memory as f64 - optimized_memory as f64) / standard_memory as f64 * 100.0;
    
    println!("   ✓ Standard memory usage: {} KB", standard_memory / 1024);
    println!("   ✓ Optimized memory usage: {} KB", optimized_memory / 1024);
    ...
    let results_match = sma_standard.len() == sma_optimized.len() &&
        sma_standard.iter().zip(sma_optimized.iter()).all(|(s, o)| {
            ((*s as f32) - o).abs() < 0.001
        });
    
    println!("   ✓ Results verification: {}", if results_match { "PASSED" } else { "FAILED" });
