use nyxs_owl::{
    forecasting::{
        strategies::{
            ensemble_strategy::{EnsembleStrategy, EnsembleStrategyConfig, EnsembleMethod},
        },
        backtest::{ForecastBacktester, BacktestConfig},
    },
...
fn create_synthetic_data(length: usize) -> Result<DataFrame> {
    ...
    .map_err(|e| nyxs_owl::simple_types::NyxsOwlError::DataError(format!("Failed to create synthetic data: {}", e)))
}
...
fn test_preset_configuration(name: &str, config: EnsembleStrategyConfig, df: &DataFrame) -> Result<()> {
    println!("\n📈 {} Configuration:", name);
    ...
    println!("  Models: ARIMA={}, ES={}, Kalman={}", 
...
fn detailed_ensemble_analysis(df: &DataFrame) -> Result<()> {
    ...
    println!("  Signal Threshold: {:.2}%", config.signal_threshold * 100.0);
    println!("  Minimum Confidence: {:.1}%", config.min_confidence * 100.0);
    println!("  Performance Window: {} periods", config.performance_window);
    
    println!("\nModel Configuration:");
    println!("  ✅ ARIMA: {}", config.model_config.use_arima);
    println!("  ✅ Exponential Smoothing: {}", config.model_config.use_exponential_smoothing);
...
fn extract_prices_for_analysis(df: &DataFrame) -> Result<Vec<f64>> {
    ...
    let prices: Vec<f64> = close_column
        .f64()
        .map_err(|e| nyxs_owl::simple_types::NyxsOwlError::DataError(format!("Failed to cast to f64: {}", e)))?
        .into_no_null_iter()
        .collect();
...
fn analyze_ensemble_performance(prices: &[f64], signals: &[Signal]) {
    ...
    let buy_signals: Vec<usize> = signals.iter()
        .enumerate()
        .filter(|(_, &signal)| signal == Signal::Buy)
        .map(|(i, _)| i)
        .collect();
    
    let sell_signals: Vec<usize> = signals.iter()
        .enumerate()
    ...
    println!("    Buy Signals at periods: {:?}", &buy_signals[..buy_signals.len().min(5)]);
    println!("    Sell Signals at periods: {:?}", &sell_signals[..sell_signals.len().min(5)]);
}

fn print_comprehensive_performance(performance: &nyxs_owl::forecasting::backtest::BacktestPerformance) {
