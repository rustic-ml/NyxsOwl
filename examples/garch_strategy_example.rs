...
fn main() -> Result<()> {
    ...
    test_preset_configuration("Volatility Trading", GarchStrategyConfig::volatility_trading(), &df)?;
...
fn test_preset_configuration(name: &str, config: GarchStrategyConfig, df: &DataFrame) -> Result<()> {
    println!("\n📈 {} Configuration:", name);
    println!("  Model Type: {:?}", config.model_type);
    println!("  GARCH Order: {}, ARCH Order: {}", config.garch_order, config.arch_order);
    println!("  Volatility Threshold: {:.1}x", config.volatility_threshold);
    println!("  Signal Threshold: {:.2}%", config.signal_threshold * 100.0);
    println!("  Volatility Targeting: {}", config.use_volatility_targeting);
    println!("  Target Volatility: {:.1}%", config.target_volatility * 100.0);
...
fn analyze_volatility_performance(config: &GarchStrategyConfig, signals: &[Signal], df: &DataFrame) -> Result<()> {
    ...
    println!("    Realized Volatility: {:.3}% daily ({:.1}% annualized)", 
             realized_vol * 100.0, realized_vol * (252.0_f64).sqrt() * 100.0);
    
    // Volatility regime analysis
    let high_vol_threshold = realized_vol * config.volatility_threshold;
    let high_vol_periods = returns.iter()
        .map(|&r| r.abs())
        .filter(|&abs_r| abs_r > high_vol_threshold)
        .count();
    
    let high_vol_percentage = high_vol_periods as f64 / returns.len() as f64 * 100.0;
    println!("    High Volatility Periods: {:.1}% of time", high_vol_percentage);
...
fn analyze_signal_volatility_timing(returns: &[f64], signals: &[Signal], avg_vol: f64, vol_threshold: f64) {
    ...
    if total_trading_signals > 0 {
        println!("      Signals in High Vol: {:.1}% ({}/{})", 
                 signals_in_high_vol as f64 / total_trading_signals as f64 * 100.0,
                 signals_in_high_vol, total_trading_signals);
        println!("      Signals in Low Vol: {:.1}% ({}/{})", 
                 signals_in_low_vol as f64 / total_trading_signals as f64 * 100.0,
                 signals_in_low_vol, total_trading_signals);
...
fn detailed_garch_analysis(df: &DataFrame) -> Result<()> {
    ...
    println!("    Orders: GARCH({}) ARCH({})", config.garch_order, config.arch_order);
    println!("    Volatility Threshold: {:.1}x average", config.volatility_threshold);
    println!("    Signal Threshold: {:.2}%", config.signal_threshold * 100.0);
...
fn analyze_comprehensive_volatility(prices: &[f64], signals: &[Signal], config: &GarchStrategyConfig) -> Result<()> {
    println!("\n🌊 Comprehensive Volatility Analysis:");
    ...
    if !rolling_volatilities.is_empty() {
        ...
        let vol_of_vol = {
            ...
            let variance = rolling_volatilities.iter()
                .map(|&v| (v - vol_mean).powi(2))
                .sum::<f64>() / rolling_volatilities.len() as f64;
            variance.sqrt()
        };
        ...
        let min_vol = rolling_volatilities.iter().cloned().fold(f64::INFINITY, f64::min);
        
        println!("  Volatility Range: {:.3}% - {:.3}%", min_vol * 100.0, max_vol * 100.0);
...
fn analyze_strategy_performance(signals: &[Signal], prices: &[f64], config: &GarchStrategyConfig) -> Result<()> {
    println!("\n📈 Strategy Performance Analysis:");
    ...
    if !window_activities.is_empty() {
        ...
        let activity_variance = {
            let variance = window_activities.iter()
                .map(|&a| (a - avg_activity).powi(2))
                .sum::<f64>() / window_activities.len() as f64;
            variance.sqrt()
        };
        
        println!("  Activity Consistency: {:.3} (lower = more consistent)", activity_variance);
    }
...
fn analyze_risk_return_profile(signals: &[Signal], prices: &[f64]) {
    ...
    if !strategy_returns.is_empty() {
        let avg_strategy_return = strategy_returns.iter().sum::<f64>() / strategy_returns.len() as f64;
        ...
        println!("    Strategy Return: {:.4}% daily ({:.1}% annualized)", 
                 avg_strategy_return * 100.0, avg_strategy_return * 252.0 * 100.0);
        println!("    Strategy Volatility: {:.3}% daily ({:.1}% annualized)", 
                 strategy_volatility * 100.0, strategy_volatility * (252.0_f64).sqrt() * 100.0);
        ...
        // Downside analysis
        let negative_returns: Vec<f64> = strategy_returns.iter()
            .filter(|&&r| r < 0.0)
            .cloned()
            .collect();
