...
use nyxs_owl::trade_math::{
    calculate_adl, calculate_cci, calculate_cmf, calculate_fibonacci_extensions,
    calculate_fibonacci_retracements, calculate_mfi, calculate_roc, calculate_supertrend,
    calculate_vroc, calculate_vwap_with_bands,
};
use polars::prelude::*;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "📊 Created sample market data with {} data points",
        market_data.height()
    );
...
fn demonstrate_advanced_oscillators(data: &DataFrame) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let high = data
        .column("high")?
        .as_series()
        .expect("Failed to get high series");
    let low = data
        .column("low")?
        .as_series()
        .expect("Failed to get low series");
    let close = data
        .column("close")?
        .as_series()
        .expect("Failed to get close series");
    let volume = data
        .column("volume")?
        .as_series()
        .expect("Failed to get volume series");

    // CCI (Commodity Channel Index)
    let cci = calculate_cci(&high, &low, &close, 20)?;
    println!(
        "✅ CCI calculated - Latest value: {:.2}",
...
fn demonstrate_volume_indicators(data: &DataFrame) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ VROC calculated - Latest value: {:.2}%",
    ...
    println!(
        "   VWAP: {:.2}",
        vwap.f64()?.get(vwap.len() - 1).unwrap_or(0.0)
    );
    println!(
        "   Upper Band: {:.2}",
        vwap_upper.f64()?.get(vwap_upper.len() - 1).unwrap_or(0.0)
    );
    println!(
        "   Lower Band: {:.2}",
        vwap_lower.f64()?.get(vwap_lower.len() - 1).unwrap_or(0.0)
    );

    // ADL (Accumulation/Distribution Line)
    let adl = calculate_adl(&high, &low, &close, &volume)?;
    println!(
        "✅ ADL calculated - Latest value: {:.2}",
...
fn demonstrate_trend_indicators(data: &DataFrame) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   SuperTrend: {:.2}",
        supertrend.f64()?.get(supertrend.len() - 1).unwrap_or(0.0)
    );
    println!(
        "   Trend Direction: {:.0}",
        trend_direction
            .f64()?
            .get(trend_direction.len() - 1)
            .unwrap_or(0.0)
    );
