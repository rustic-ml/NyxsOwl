...
use nyxs_owl::forecasting::strategies::{
    CopulaStrategy, CopulaStrategyConfig, CopulaStrategyType, CopulaType,
};
...
fn main() -> Result<()> {
    ...
    test_strategy_type(
        "Pairs Trading",
        CopulaStrategyConfig::pairs_trading("AAPL", "MSFT"),
        &combined_df,
    )?;
    test_strategy_type(
...
fn analyze_signals(signals: &[Signal], strategy_name: &str) {
    ...
    match activity_rate {
        ...
        rate if rate < 10.0 => {
            println!("    📈 Moderate Activity - Typical correlation variations")
        }
...
fn analyze_correlation_patterns(df: &DataFrame, price_columns: &[String]) -> Result<()> {
    ...
    if !rolling_correlations.is_empty() {
        let avg_rolling_corr =
            rolling_correlations.iter().sum::<f64>() / rolling_correlations.len() as f64;
        let max_corr = rolling_correlations
            .iter()
            .cloned()
            .fold(f64::NEG_INFINITY, f64::max);
        let min_corr = rolling_correlations
            .iter()
            .cloned()
...
fn analyze_correlation_regimes(correlations: &[f64]) {
    ...
    let high_corr_periods = correlations
        .iter()
        .filter(|&&c| c > high_corr_threshold)
        .count();
    let low_corr_periods = correlations
        .iter()
        .filter(|&&c| c.abs() < low_corr_threshold)
        .count();
    let medium_corr_periods = correlations.len() - high_corr_periods - low_corr_periods;

    println!(
        "      High Correlation (>{:.1}): {:.1}%",
        high_corr_threshold,
        high_corr_periods as f64 / correlations.len() as f64 * 100.0
    );
    println!(
        "      Medium Correlation: {:.1}%",
...
fn analyze_strategy_effectiveness(
    signals: &[Signal],
    df: &DataFrame,
...
fn analyze_signal_timing_effectiveness(
    signals: &[Signal],
    df: &DataFrame,
    price_columns: &[String],
) -> Result<()> {
    ...
    if total_trading_signals > 0 {
        ...
        println!(
            "    Forecast Accuracy: {:.1}% ({}/{})",
            accuracy, correct_signals, total_trading_signals
        );
    ...
    let data: Vec<f64> = column
        ...
        .ok_or_else(|| {
            nyxs_owl::simple_types::NyxsOwlError::DataError(
                "Column contains null values".to_string(),
            )
        })?;
