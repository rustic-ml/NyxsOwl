use nyxs_owl::forecasting::strategies::{RegimeSwitchingStrategy, RegimeSwitchingConfig, RegimeSwitchingType, MarketRegime};
...
fn main() -> Result<()> {
    ...
    test_regime_model("Markov Switching", RegimeSwitchingType::MarkovSwitching, &df)?;
    test_regime_model("Higher-Order (3)", RegimeSwitchingType::HigherOrder(3), &df)?;
    test_regime_model("Duration-Dependent", RegimeSwitchingType::DurationDependent, &df)?;
    ...
    test_preset_configuration("Bull/Bear Market", RegimeSwitchingConfig::bull_bear_market(), &df)?;
    test_preset_configuration("Volatility Regimes", RegimeSwitchingConfig::volatility_regimes(), &df)?;
    test_preset_configuration("Crisis Detection", RegimeSwitchingConfig::crisis_detection(), &df)?;
...
fn test_preset_configuration(name: &str, config: RegimeSwitchingConfig, df: &DataFrame) -> Result<()> {
    println!("\n📈 {} Configuration:", name);
...
fn analyze_signal_persistence(signals: &[Signal]) {
    ...
    let buy_runs: Vec<usize> = signal_runs.iter()
        .filter(|(sig, _)| *sig == Signal::Buy)
        .map(|(_, len)| *len)
        .collect();
    
    let sell_runs: Vec<usize> = signal_runs.iter()
        .filter(|(sig, _)| *sig == Signal::Sell)
        .map(|(_, len)| *len)
        .collect();
    
    let hold_runs: Vec<usize> = signal_runs.iter()
...
fn simulate_regime_detection(returns: &[f64], config: &RegimeSwitchingConfig) {
    ...
    for &ret in returns.iter().take(returns.len().min(config.regime_window * 3)) {
        let volatility = ret.abs();
        
        let regime = if ret > config.return_threshold * 2.0 && volatility < config.volatility_threshold {
            MarketRegime::Bull
    ...
    for (regime, count) in regime_counts {
        ...
        println!("      {}: {:.1}% ({} periods)", regime.as_str(), percentage, count);
    }
...
fn analyze_regime_transitions(returns: &[f64], config: &RegimeSwitchingConfig) {
    ...
    let regimes: Vec<MarketRegime> = returns.iter().map(|&ret| {
        let volatility = ret.abs();
    ...
    println!("      Transition Rate: {:.1}%", transitions as f64 / regimes.len() as f64 * 100.0);
...
fn detailed_regime_analysis(df: &DataFrame) -> Result<()> {
    ...
    println!("    Confidence Level: {:.1}%", config.regime_confidence * 100.0);
...
fn analyze_comprehensive_regimes(prices: &[f64], signals: &[Signal], config: &RegimeSwitchingConfig) -> Result<()> {
    println!("\n🎯 Comprehensive Regime Analysis:");
    ...
    if !rolling_means.is_empty() && !rolling_vols.is_empty() {
        let mean_stability = {
            ...
            let variance = rolling_means.iter()
                ...
                .sum::<f64>() / rolling_means.len() as f64;
        ...
        let vol_stability = {
            ...
            let vol_variance = rolling_vols.iter()
                ...
                .sum::<f64>() / rolling_vols.len() as f64;
            1.0 - (vol_variance.sqrt() / mean_vol).min(1.0)
        };
        
        println!("    Return Stability: {:.3} (higher = more stable)", mean_stability);
        println!("    Volatility Stability: {:.3} (higher = more stable)", vol_stability);
...
fn analyze_signal_regime_correlation(returns: &[f64], signals: &[Signal], config: &RegimeSwitchingConfig) {
    ...
    if total_trading_signals > 0 {
        println!("    Signals in Bull Conditions: {:.1}%", 
                 bull_signals as f64 / total_trading_signals as f64 * 100.0);
        println!("    Signals in Bear Conditions: {:.1}%", 
                 bear_signals as f64 / total_trading_signals as f64 * 100.0);
        println!("    Signals in Neutral Conditions: {:.1}%", 
...
fn analyze_strategy_adaptation(config: &RegimeSwitchingConfig, signals: &[Signal]) {
    ...
    for strategy in &config.regime_strategies {
        ...
        println!("      Position Multiplier: {:.1}x", strategy.position_multiplier);
    ...
    println!("  Adaptation Responsiveness: {:.1}% signal change rate", adaptation_rate);
