...
use nyxs_owl::forecasting::strategies::{
    AdaptiveEnsembleConfig, AdaptiveEnsembleStrategy, ArimaStrategy, ArimaStrategyConfig,
    MarketRegime,
};
...
fn demo_regime_adaptation(df: &DataFrame) -> Result<()> {
    ...
    for (start_idx, regime, duration) in regimes {
        ...
        println!(
            "      {:?} regime: {}% of data (starting at index {})",
...
fn demo_quality_monitoring(df: &DataFrame) -> Result<()> {
    ...
    for i in (window_size..prices.len()).step_by(50) {
        let window_data =
            create_window_dataframe(&prices[i - window_size..i + 1], i - window_size)?;
        ...
        if accuracy < 0.6 {
            println!(
                "      ⚠️  Quality Alert at index {}: Accuracy {:.1}%",
                i,
                accuracy * 100.0
            );
        } else if accuracy > 0.8 {
            println!(
                "      ✅ High Quality at index {}: Accuracy {:.1}%",
                i,
                accuracy * 100.0
            );
        }
...
fn calculate_window_accuracy(signals: &[Signal], prices: &[f64]) -> f64 {
    ...
    for i in 0..signals.len().min(prices.len() - 1) {
        let actual_direction = if prices[i + 1] > prices[i] {
            1
