...
use nyxs_owl::performance_utils::{SimdMath, SimdBenchmark};
...
fn test_simd_functions() {
    ...
    let test_data_small: Vec<f64> = (0..100).map(|i| (i as f64 * 0.05).sin() + 0.1 * (i as f64)).collect();
    let test_data_large: Vec<f64> = (0..10000).map(|i| (i as f64 * 0.001).cos() * 100.0 + i as f64 * 0.01).collect();
...
fn forecasting_scenario_demo() {
    ...
    println!("     - Annualized volatility: {:.2}%", return_volatility * (252.0_f64.sqrt()) * 100.0);
    ...
    if annualized_volatility > 0.25 {
        println!("   • High volatility detected ({:.1}% annualized)", annualized_volatility * 100.0);
        println!("   • Enhanced risk management recommended");
    } else if annualized_volatility < 0.15 {
        println!("   • Low volatility environment ({:.1}% annualized)", annualized_volatility * 100.0);
        println!("   • Suitable for momentum strategies");
    } else {
        println!("   • Moderate volatility ({:.1}% annualized)", annualized_volatility * 100.0);
