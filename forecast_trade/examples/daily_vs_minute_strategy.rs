...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Need to import ForecastModel trait to use train method
    ...
    println!("  Max drawdown: {:.1}%", daily_mr_results.max_drawdown * 100.0);
    ...
    println!("  Max drawdown: {:.1}%", minute_mr_results.max_drawdown * 100.0);
    println!("  Sharpe ratio: {:.2}", minute_mr_results.performance_metrics.sharpe_ratio.unwrap_or(0.0));
    ...
    println!("  Daily data can be converted to OHLCV format using TimeSeriesData::to_daily_ohlcv()");
    println!("  Minute data can be converted to OHLCV format using TimeSeriesData::to_minute_ohlcv()");
...
fn print_signal_stats(strategy_name: &str, signals: &[TradingSignal]) {
    ...
    println!("  Buy signals: {} ({:.1}%)", buy_count, (buy_count as f64 / total as f64) * 100.0);
    println!("  Sell signals: {} ({:.1}%)", sell_count, (sell_count as f64 / total as f64) * 100.0);
    println!("  Hold signals: {} ({:.1}%)", hold_count, (hold_count as f64 / total as f64) * 100.0);
}
