...
impl CacheOptimizedTimeSeries {
    ...
    pub fn push(&mut self, timestamp: u64, open: f64, high: f64, low: f64, close: f64, volume: u64) {
        ...
        let volatility = if self.len >= 20 {
            ...
            let variance: f32 = window_returns.iter()
                .map(|r| (r - mean_return).powi(2))
                .sum::<f32>() / (window_returns.len() - 1) as f32;
            variance.sqrt()
        } else {
            0.0
        };
    ...
    pub fn closes(&self) -> &[f32] { &self.closes[..self.len] }
    pub fn opens(&self) -> &[f32] { &self.opens[..self.len] }
    pub fn highs(&self) -> &[f32] { &self.highs[..self.len] }
    pub fn lows(&self) -> &[f32] { &self.lows[..self.len] }
    pub fn volumes(&self) -> &[u32] { &self.volumes[..self.len] }
    pub fn returns(&self) -> &[f32] { &self.returns[..self.len] }
    pub fn log_returns(&self) -> &[f32] { &self.log_returns[..self.len] }
    pub fn volatility(&self) -> &[f32] { &self.volatility[..self.len] }
    pub fn timestamps(&self) -> &[u64] { &self.timestamps[..self.len] }
    ...
    pub fn memory_usage(&self) -> usize {
        ...
        (self.opens.capacity() + self.highs.capacity() + self.lows.capacity() + 
         self.closes.capacity() + self.returns.capacity() + self.log_returns.capacity() + 
         self.volatility.capacity()) * std::mem::size_of::<f32>() +
    ...
    pub fn add_price(&mut self, price: f32) {
        ...
        self.push(timestamp, price as f64, price as f64, price as f64, price as f64, 1000);
    }
    ...
    V: Clone,
{
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        ...
        for _ in 0..bucket_count {
            buckets.push(CacheFriendlyBucket { entries: Vec::new() });
        }
    ...
    fn resize(&mut self) {
        ...
        for _ in 0..new_capacity {
            self.buckets.push(CacheFriendlyBucket { entries: Vec::new() });
        }
    ...
    pub fn memory_usage(&self) -> usize {
        ...
        self.buckets.iter().map(|bucket| 
            bucket.entries.capacity() * std::mem::size_of::<(K, V, u64)>()
        ).sum::<usize>()
    }
}
