...
impl<M: ForecastModel> ForecastStrategy for MeanReversionStrategy<M> {
    fn generate_signals(&self, data: &TimeSeriesData) -> Result<Vec<TradingSignal>> {
        ...
        let predictions_result = trained_model.forecast(data, 1)?;
        let predictions = predictions_result.forecasts;

        // Calculate the standard deviation
        let mut std_dev = 0.0;
        let mut sum_squared_diff = 0.0;
        for i in 0..prices.len() {
            let diff = prices[i] - predictions[i];
    ...
    fn backtest_with_params(
        ...
        Ok(BacktestResult {
            ...
            trades: total_trades,
            performance_metrics: Some(crate::forecast_trade::strategies::PerformanceMetrics {
                sharpe_ratio,
