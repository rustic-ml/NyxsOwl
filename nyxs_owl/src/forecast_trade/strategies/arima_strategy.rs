...
use crate::forecast_trade::models::oxidiviner::ArimaAdapter;
...
#[derive(Clone)]
pub struct ArimaStrategy<M: ForecastModel + Clone + ForecastStrategy + 'static> {
...
impl<M: ForecastModel + Clone + ForecastStrategy + 'static> ArimaStrategy<M> {
    ...
    pub fn new(model: M, threshold: f64) -> Result<Self> {
        ...
        let forecast_horizon = match ForecastModel::time_granularity(&model) {
...
impl<M: ForecastModel + Clone + ForecastStrategy + 'static> ForecastStrategy for ArimaStrategy<M> {
    fn generate_signals(&self, data: &TimeSeriesData) -> Result<Vec<TradingSignal>> {
        ...
        let next_price = forecast.forecasts[0];
