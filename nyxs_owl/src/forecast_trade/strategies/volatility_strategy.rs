...
impl<M: ForecastModel + Clone + ForecastStrategy + 'static> VolatilityStrategy<M> {
    ...
    pub fn new(model: M, threshold_multiplier: f64) -> Result<Self> {
        ...
        let lookback_window = match ForecastModel::time_granularity(&model) {
    ...
    fn calculate_returns(&self, prices: &[f64]) -> Vec<f64> {
        ...
        returns.extend(prices.windows(2).map(|w| (w[1] / w[0]) - 1.0));
    ...
    for VolatilityStrategy<M>
{
    fn generate_signals(&self, data: &TimeSeriesData) -> Result<Vec<TradingSignal>> {
        ...
        let forecasted_volatility = forecast.forecasts[0];
...
pub fn create_garch_strategy(
    ...
    let model = GarchAdapter::with_params(p, q)?.with_granularity(granularity);
