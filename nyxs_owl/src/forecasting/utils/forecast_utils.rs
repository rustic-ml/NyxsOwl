...
pub fn extract_numeric_series(df: &DataFrame, column_name: &str) -> Result<Vec<f64>> {
    let series = df.column(column_name)
        .map_err(|e| NyxsOwlError::DataError(format!("Column '{}' not found: {}", column_name, e)))?;
...
pub fn validate_dataframe(
    ...
    if df.height() < min_rows {
        return Err(NyxsOwlError::MissingData(format!(
            ...
            strategy_name, min_rows, df.height()
        )));
    }
    ...
    for col_name in required_columns {
        if df.column(col_name).is_err() {
            return Err(NyxsOwlError::DataError(format!(
                ...
                strategy_name, col_name, df.get_column_names()
            )));
        }
    }
...
impl SimpleStats {
    pub fn calculate(data: &[f64]) -> Result<Self> {
        if data.is_empty() {
            return Err(NyxsOwlError::DataError("Cannot calculate stats for empty data".to_string()));
        }
