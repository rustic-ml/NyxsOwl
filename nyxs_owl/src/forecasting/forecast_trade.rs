...
impl From<InternalForecastError> for NyxsOwlError {
    fn from(err: InternalForecastError) -> Self {
        match err {
            InternalForecastError::ArimaError(s) => NyxsOwlError::StrategyError(format!("ARIMA forecasting error: {}", s)),
            InternalForecastError::InputError(s) => NyxsOwlError::DataError(format!("Input data error for forecast: {}", s)),
            InternalForecastError::ModelError(s) => NyxsOwlError::StrategyError(format!("Underlying model error: {}", s)),
        }
    }
}
...
pub fn forecast_arima(
...
) -> NyxsOwlResult<Vec<f64>> { // Changed to NyxsOwlResult (which implies NyxsOwlError)
    ...
    if let Err(stability_error) = validate_data_stability(&final_data) {
        warn!("forecast_arima: Data stability check failed: {}", stability_error);
        // Continue with warning but monitor results more carefully
    }
    
    match oxidiviner::quick::arima_forecast_custom(timestamps, final_data.clone(), periods_to_forecast, p, d, q) {
        Ok(forecasts) => {
            ...
            if let Err(reasonableness_error) = validate_forecast_reasonableness(&data, &forecasts) {
                warn!("forecast_arima: Forecast reasonableness check failed: {}", reasonableness_error);
                // Continue with warning - may still be valid in volatile markets
            }
            
            debug!("OxiDiviner arima_forecast_custom successful. Forecast len: {}", forecasts.len());
            Ok(forecasts)
        }
        Err(e) => { 
            error!(
...
fn detect_and_handle_outliers(data: &[f64]) -> NyxsOwlResult<Vec<f64>> {
    ...
    for &idx in &outlier_indices {
        ...
        let mut neighbors: Vec<f64> = data[start..end].iter()
            .enumerate()
        ...
        if !neighbors.is_empty() {
            ...
            cleaned[idx] = median;
            debug!("Replaced outlier at index {}: {} -> {}", idx, data[idx], median);
        }
    }
    
    debug!("Detected and corrected {} outliers in time series data", outlier_indices.len());
    Ok(cleaned)
}
...
fn validate_data_stability(data: &[f64]) -> Result<(), String> {
    ...
    if mean != 0.0 {
        ...
        if cv > 2.0 { // Very high relative volatility
    ...
    if var1 > 0.0 && var2 > 0.0 {
        ...
        if variance_ratio > 4.0 || variance_ratio < 0.25 {
            return Err(format!("Significant variance change detected (ratio: {:.3})", variance_ratio));
        }
    }
    ...
    for (i, &forecast) in forecasts.iter().enumerate() {
        ...
        if (forecast - hist_mean).abs() > reasonable_bound {
            return Err(format!(
                ...
                forecast, i, (forecast - hist_mean).abs() / hist_std
            ));
        }
...
mod tests {
    ...
    fn is_forecast_related_error(err: &CrateNyxsOwlError) -> bool {
        matches!(err, CrateNyxsOwlError::DataError(_) | CrateNyxsOwlError::StrategyError(_))
    }

    fn create_sample_timestamps(n: usize) -> Vec<DateTime<Utc>> {
        let start_time = Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 0).unwrap();
        (0..n).map(|i| start_time + Duration::days(i as i64)).collect()
    }
    ...
    fn test_forecast_arima_simple_success() {
        ...
        let data: Vec<f64> = (1..=n).map(|x| x as f64 * 1.5 + ((x % 5) as f64 * 0.5) ).collect();
        let order = (1, 1, 0);
        let periods = 3;
        match forecast_arima(timestamps, data, order, periods) {
            ...
            Err(e) => panic!("forecast_arima_custom failed for simple success case: {:?}", e),
        }
    }
    ...
    fn test_forecast_arima_pdq_success() {
        ...
        let data: Vec<f64> = vec![
            10.0, 10.2, 10.4, 10.3, 10.5, 10.6, 10.8, 10.7, 10.9, 11.0,
            11.2, 11.3, 11.5, 11.4, 11.6, 11.7, 11.9, 11.8, 12.0, 12.2,
            12.1, 12.3, 12.4, 12.6, 12.5, 12.7, 12.8, 13.0, 12.9, 13.1,
            13.3, 13.2, 13.4, 13.5, 13.7, 13.6, 13.8, 13.9, 14.1, 14.0
        ];
        let order = (2, 1, 1);
        let periods = 5;
        match forecast_arima(timestamps, data, order, periods) {
            ...
            Err(e) => panic!("forecast_arima_custom failed for ARIMA(2,1,1) case: {:?}", e),
        }
    }
    ...
    fn test_forecast_arima_empty_data_or_timestamps() {
        ...
        match forecast_arima(vec![], data.clone(), (1,0,0), 1) {
            Err(ref e) if is_forecast_related_error(e) => { 
                 assert!(e.to_string().contains("Input data or timestamps slice is empty"));
            }
    ...
    fn test_forecast_arima_mismatched_lengths() {
        ...
        match forecast_arima(timestamps, data, (1,0,0), 1) {
            Err(ref e) if is_forecast_related_error(e) => {
                assert!(e.to_string().contains("Data and timestamps lengths do not match"));
            }
    ...
    fn test_forecast_arima_insufficient_data_for_order() {
        ...
        match forecast_arima(timestamps, data, order, 1) {
            Err(ref e) if is_forecast_related_error(e) => {
                 println!("Received expected forecast error for insufficient data: {}", e);
                 let err_string = e.to_string().to_lowercase();
                 assert!(err_string.contains("oxidiviner") || err_string.contains("model") || err_string.contains("arima") || err_string.contains("series length") || err_string.contains("underlying model error"));
            }
            res => panic!("Expected StrategyError or DataError for insufficient data, got {:?}", res),
        }
    }
    ...
    fn test_forecast_arima_non_finite_input_data() {
        ...
        match forecast_arima(timestamps, data, (1,0,0), 1) {
            Err(ref e) if is_forecast_related_error(e) => {
                assert!(e.to_string().contains("Input data contains non-finite values"));
            }
            res => panic!("Expected DataError for non-finite input data, got {:?}", res),
        }
    }
    ...
    fn test_constant_data_leading_to_potential_fit_issues() {
        ...
        match forecast_arima(timestamps.clone(), data.clone(), order, periods) {
            Ok(forecasts) => {
                assert_eq!(forecasts.len(), periods);
                assert!(!forecasts.iter().any(|f| !f.is_finite()));
                println!("Forecasts for constant data (order {:?}): {:?}", order, forecasts);
            ...
            Err(e) => {
                ...
                // For now, panic if it's not Ok, as OxiDiviner seems to handle it.
                 panic!("forecast_arima_custom failed for constant data case: {:?}", e)
            }
        }
        ...
        let order_d1 = (1,1,0);
         match forecast_arima(timestamps, data, order_d1, periods) {
            Ok(forecasts) => {
                ...
                assert!(forecasts.iter().all(|&f| (f - 5.0).abs() < 1e-9), "Forecasts: {:?}", forecasts);
                println!("Forecasts for constant data (order {:?}): {:?}", order_d1, forecasts);
            }
            Err(e) => panic!("forecast_arima_custom failed for constant data with d=1 (order {:?}): {:?}", order_d1, e),
        }
    }
} 