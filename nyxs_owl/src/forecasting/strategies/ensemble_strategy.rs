...
use crate::forecasting::strategies::{
    ArimaStrategy, ArimaStrategyConfig,
...
impl EnsembleStrategy {
    ...
    pub fn generate_signals(
        ...
        let individual_signals = self.collect_individual_signals(df, price_column, timestamp_column)?;
    ...
    fn collect_individual_signals(
        ...
        if self.config.model_config.use_arima {
            let mut arima_strategy = ArimaStrategy::new(self.config.model_config.arima_config.clone());
            let arima_signals = arima_strategy.generate_signals(df, price_column, timestamp_column)
        ...
        if self.config.model_config.use_exponential_smoothing {
            let es_strategy = ExponentialSmoothingStrategy::new(
                self.config.model_config.exponential_smoothing_config.clone()
            );
            let es_signals = es_strategy.generate_signals(df, price_column, timestamp_column)
                .map_err(|e| NyxsOwlError::StrategyError(format!("Exponential Smoothing model failed: {}", e)))?;
            signals_map.insert("ExponentialSmoothing".to_string(), es_signals);
        }
        
        // Kalman Filter model
        if self.config.model_config.use_kalman {
            let kalman_strategy = KalmanStrategy::new(self.config.model_config.kalman_config.clone());
            let kalman_signals = kalman_strategy.generate_signals(df, price_column, timestamp_column)
                .map_err(|e| NyxsOwlError::StrategyError(format!("Kalman Filter model failed: {}", e)))?;
    ...
    fn aggregate_signals(&self, signals_map: &HashMap<String, Vec<Signal>>) -> Result<Vec<Signal>> {
        let signal_length = signals_map.values().next()
    ...
    fn weighted_average_aggregation(
        ...
        for i in 0..final_signals.len() {
            ...
            final_signals[i] = if weighted_buy > self.config.min_confidence && weighted_buy > weighted_sell {
                Signal::Buy
    ...
    fn stacking_aggregation(
        ...
        if lookback_window > 10 {
            self.calculate_stacking_weights(signals_map, &models, &mut model_weights, lookback_window)?;
        }
        ...
        for i in 0..final_signals.len() {
            ...
            for model_name in &models {
                ...
                let adjusted_weight = base_weight * (1.0 + agreement_factor * 0.5 + trend_factor * 0.3);
            ...
            if total_weight > 0.0 {
                ...
                final_signals[i] = if buy_confidence > confidence_threshold && weighted_buy_score > weighted_sell_score {
                    Signal::Buy
                } else if sell_confidence > confidence_threshold && weighted_sell_score > weighted_buy_score {
                    Signal::Sell
                } else {
                    Signal::Hold
                };
...
mod tests {
    ...
    fn test_preset_configurations() {
        ...
        assert!(matches!(balanced.method, EnsembleMethod::WeightedAverage(_)));
    ...
    fn test_median_ensemble() {
        ...
        let prices: Vec<f64> = (0..150).map(|i| 100.0 + (i as f64 * 0.1).sin() * 5.0).collect();
        let df = create_test_dataframe(prices);
