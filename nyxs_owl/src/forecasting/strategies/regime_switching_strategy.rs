...
impl RegimeSwitchingStrategy {
    ...
    fn higher_order_detection(
        ...
        let smoothed_regimes =
            self.apply_higher_order_smoothing(&basic_detection.regimes, order)?;
    ...
    fn classify_regime(&self, return_val: f64, volatility: f64) -> MarketRegime {
        // Crisis detection
        if volatility > self.config.volatility_threshold * 3.0
            && return_val < -self.config.return_threshold * 5.0
        {
    ...
    fn apply_higher_order_smoothing(
        &self,
    ...
    fn generate_regime_based_signals(
        ...
        for (i, &regime) in regime_detection.regimes.iter().enumerate() {
            if let Some(regime_strategy) = self
                .config
                .regime_strategies
                .iter()
    ...
    fn generate_signal_for_regime(
        ...
        let signal = if regime_strategy.use_trend_following {
            ...
            if index >= 10 {
                ...
                let avg_return =
                    returns[index.saturating_sub(10)..index].iter().sum::<f64>() / 10.0;
...
mod tests {
    ...
    fn test_regime_switching_config_validation() {
        // Valid configuration
        let config =
            RegimeSwitchingConfig::new(RegimeSwitchingType::MarkovSwitching, 3, 60, 0.02, 0.001);
        assert!(config.is_ok());

        // Invalid number of regimes
        let config =
            RegimeSwitchingConfig::new(RegimeSwitchingType::MarkovSwitching, 1, 60, 0.02, 0.001);
        assert!(config.is_err());

        // Invalid regime window
        let config =
            RegimeSwitchingConfig::new(RegimeSwitchingType::MarkovSwitching, 3, 10, 0.02, 0.001);
        assert!(config.is_err());

        // Invalid higher-order parameter
        let config =
            RegimeSwitchingConfig::new(RegimeSwitchingType::HigherOrder(0), 3, 60, 0.02, 0.001);
        assert!(config.is_err());
    }
    ...
    fn test_rolling_statistics_calculation() {
        ...
        let (rolling_returns, rolling_vol) =
            strategy.calculate_rolling_statistics(&returns).unwrap();
    ...
    fn test_higher_order_smoothing() {
        ...
        let regime_changes = smoothed
            .windows(2)
            .filter(|window| window[0] != window[1])
            .count();
        let original_changes = regimes
            .windows(2)
            .filter(|window| window[0] != window[1])
            .count();
