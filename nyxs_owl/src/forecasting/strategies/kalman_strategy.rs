...
impl KalmanStrategy {
    ...
    fn extract_timestamps(&self, df: &DataFrame, timestamp_column: &str) -> Result<Vec<String>> {
        ...
        let timestamps: Vec<String> = if column.dtype() == &DataType::String {
            column
                .str()
                .map_err(|e| NyxsOwlError::DataError(format!("Failed to convert timestamps: {}", e)))?
                .into_iter()
                .collect::<Option<Vec<&str>>>()
                .ok_or_else(|| NyxsOwlError::DataError("Timestamp column contains null values".to_string()))?
                .into_iter()
                .map(|s| s.to_string())
                .collect()
        } else {
    ...
    fn apply_kalman_filter(&self, prices: &[f64]) -> Result<KalmanFilterResults> {
        ...
        for (i, &observation) in prices.iter().enumerate() {
            ...
            if i > 0 && i % 10 == 0 && recent_innovations.len() >= 5 {
                let avg_innovation = recent_innovations.iter().sum::<f64>() / recent_innovations.len() as f64;
                let innovation_std = calculate_std_dev(&recent_innovations);
                
                // Adapt observation noise based on recent innovation magnitude
                let innovation_factor = (avg_innovation / adaptive_observation_noise.sqrt()).clamp(0.5, 2.0);
                adaptive_observation_noise = (self.config.observation_noise * innovation_factor).clamp(
                ...
                debug!("Adaptive noise update at step {}: obs_noise={:.6}, proc_noise={:.6}", 
                       i, adaptive_observation_noise, adaptive_process_noise);
            }
        }
        
        // Calculate enhanced trend estimates
        results.trend_estimates = self.calculate_enhanced_trend_estimates(&results.filtered_prices)?;
    ...
    fn calculate_enhanced_trend_estimates(&self, filtered_prices: &[f64]) -> Result<Vec<f64>> {
        ...
        for i in 2..filtered_prices.len() {
            ...
            let xy_sum = x_values.iter().zip(window.iter()).map(|(&x, &y)| x * y).sum::<f64>();
    ...
    fn apply_enhanced_smoothing(&self, trends: &[f64]) -> Result<Vec<f64>> {
        ...
        for i in 1..trends.len() {
            ...
            let recent_window = (i.saturating_sub(5)..i).map(|j| trends[j]).collect::<Vec<_>>();
            if recent_window.len() >= 3 {
                ...
                if (trends[i] - median).abs() > 3.0 * mad && mad > 1e-8 {
                    ...
                    smoothed[i] = damping_factor * raw_smoothed + (1.0 - damping_factor) * smoothed[i-1];
                } else {
    ...
    fn calculate_trend_confidence(
        ...
        let current_direction = if recent_trends[recent_trends.len()-1] > 0.0 { 1.0 } else { -1.0 };
        let consistent_count = recent_trends.iter()
            .map(|&trend| if (trend > 0.0 && current_direction > 0.0) || (trend < 0.0 && current_direction < 0.0) { 1.0 } else { 0.0 })
            .sum::<f64>();
        ...
        let trend_variance = recent_trends.iter()
        ...
        let confidence = (direction_consistency * 0.7 + magnitude_consistency * 0.3).clamp(0.0, 1.0);
