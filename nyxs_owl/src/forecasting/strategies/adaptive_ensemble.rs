...
impl ModelPerformance {
    ...
    pub fn update_performance(&mut self, accuracy: f64, regime: &MarketRegime, confidence: f64, timestamp: usize) {
...
impl AdaptiveEnsembleStrategy {
    ...
    fn generate_model_forecasts_with_oxidiviner(&self, data: &[f64]) -> Result<Vec<EnhancedForecast>> {
    ...
    fn try_oxidiviner_arima(&self, data: &[f64], p: usize, d: usize, q: usize) -> Result<(f64, f64)> {
        ...
        match oxidiviner::quick::arima_forecast_custom(
            ...
            Ok(forecasts) => {
                if forecasts.is_empty() || !forecasts[0].is_finite() {
                    return Err(NyxsOwlError::ModelError("Invalid ARIMA forecast".to_string()));
                }
            ...
            Err(e) => Err(NyxsOwlError::ModelError(format!("OxiDiviner ARIMA error: {}", e)))
        }
    }
    ...
    fn calculate_trend_strength(&self, prices: &[f64]) -> f64 {
        ...
        let xy_sum = prices.iter().enumerate()
            .map(|(i, &p)| i as f64 * p).sum::<f64>();
        let x_sq_sum = (0..prices.len())
            .map(|i| (i as f64).powi(2)).sum::<f64>();
    ...
    fn combine_forecasts(&self, forecasts: &[EnhancedForecast]) -> EnhancedForecast {
        ...
        for forecast in forecasts {
            let weight = self.model_weights.get(&forecast.model_name).copied().unwrap_or(1.0);
    ...
    fn update_performance_tracking(&mut self, forecasts: &[EnhancedForecast], actual: f64, previous: f64) {
        ...
        for forecast in forecasts {
            ...
            let performance_vec = self.performance_history
            ...
            if performance_vec.len() >= 10 {
                let recent_performance = performance_vec.iter().sum::<f64>() / performance_vec.len() as f64;
                let current_weight = self.model_weights.get(&forecast.model_name).copied().unwrap_or(1.0);
                let new_weight = current_weight * self.config.weight_decay_factor + recent_performance * (1.0 - self.config.weight_decay_factor);
                self.model_weights.insert(forecast.model_name.clone(), new_weight.clamp(0.1, 2.0));
            }
        }
    }
...
mod tests {
    ...
    fn test_regime_detection() {
        ...
        assert!(matches!(regime, MarketRegime::LowVolatility | MarketRegime::Trending));
    }
