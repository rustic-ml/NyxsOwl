...
impl CopulaStrategy {
    ...
    pub fn generate_signals(
        ...
        let signals = match self.config.strategy_type {
            CopulaStrategyType::PairsTrading => {
                self.generate_pairs_trading_signals(&price_data, &returns_data, &dependency_analysis)?
            },
            ...
            CopulaStrategyType::RiskManagement => {
                self.generate_risk_management_signals(&price_data, &returns_data, &dependency_analysis)?
            },
        };
    ...
    fn extract_multi_asset_prices(
        ...
        for col_name in price_columns {
            let column = df.column(col_name)
                .map_err(|e| NyxsOwlError::DataError(format!("Failed to get column {}: {}", col_name, e)))?;
            
            let prices: Vec<f64> = column
                .f64()
                .map_err(|e| NyxsOwlError::DataError(format!("Failed to convert {} to f64: {}", col_name, e)))?
                .into_iter()
                .collect::<Option<Vec<f64>>>()
                .ok_or_else(|| NyxsOwlError::DataError(format!("Column {} contains null values", col_name)))?;
        ...
        for (asset, prices) in price_data {
            if prices.len() < 2 {
                return Err(NyxsOwlError::DataError(
                    format!("Need at least 2 prices for {} to calculate returns", asset)
                ));
            }
    ...
    fn analyze_dependencies(
        ...
        for (asset1, asset2) in &self.config.asset_pairs {
            if let (Some(returns1), Some(returns2)) = (returns_data.get(asset1), returns_data.get(asset2)) {
                ...
                let (copula_param, tail_deps) = self.estimate_copula_parameters(returns1, returns2)?;
    ...
    fn generate_pairs_trading_signals(
        ...
        let data_length = price_data.values().next()
        ...
        for (asset1, asset2) in &self.config.asset_pairs {
            if let (Some(prices1), Some(prices2)) = (price_data.get(asset1), price_data.get(asset2)) {
                if let Some(&correlation) = dependency_analysis.correlations.get(&(asset1.clone(), asset2.clone())) {
                    ...
                    for i in self.config.rolling_window..data_length {
                        ...
                        let spreads: Vec<f64> = (window_start..i)
                            .map(|j| prices1[j] - prices2[j])
                            .collect();
    ...
    fn generate_stat_arb_signals(
        ...
        let data_length = returns_data.values().next()
        ...
        for i in self.config.rolling_window..data_length {
            ...
            for (asset1, asset2) in &self.config.asset_pairs {
                if let (Some(returns1), Some(returns2)) = (returns_data.get(asset1), returns_data.get(asset2)) {
                    ...
                    if let Ok(recent_corr) = self.calculate_correlation(recent_returns1, recent_returns2) {
                        if let Some(&historical_corr) = dependency_analysis.correlations.get(&(asset1.clone(), asset2.clone())) {
    ...
    fn generate_portfolio_signals(
        ...
        for i in self.config.rolling_window..data_length {
            ...
            if !correlation_changes.is_empty() {
                let avg_change = correlation_changes.iter().sum::<f64>() / correlation_changes.len() as f64;
    ...
    fn generate_risk_management_signals(
        ...
        for i in self.config.rolling_window..data_length {
            ...
            for (asset1, asset2) in &self.config.asset_pairs {
                if let Some((lower_tail, upper_tail)) = dependency_analysis.tail_dependencies.get(&(asset1.clone(), asset2.clone())) {
...
mod tests {
    ...
    fn test_preset_configurations() {
        ...
        assert!(matches!(pairs_config.strategy_type, CopulaStrategyType::PairsTrading));
        assert_eq!(pairs_config.asset_pairs.len(), 1);

        let stat_arb_config = CopulaStrategyConfig::statistical_arbitrage(
            vec![("A".to_string(), "B".to_string())]
        );
        assert!(matches!(stat_arb_config.strategy_type, CopulaStrategyType::StatisticalArbitrage));

        let portfolio_config = CopulaStrategyConfig::portfolio_optimization(
            vec!["A".to_string(), "B".to_string(), "C".to_string()]
        );
        assert!(matches!(portfolio_config.strategy_type, CopulaStrategyType::PortfolioOptimization));
        assert_eq!(portfolio_config.asset_pairs.len(), 3); // 3 choose 2 = 3 pairs

        let risk_config = CopulaStrategyConfig::risk_management(
            vec![("A".to_string(), "B".to_string())]
        );
        assert!(matches!(risk_config.strategy_type, CopulaStrategyType::RiskManagement));
    }
