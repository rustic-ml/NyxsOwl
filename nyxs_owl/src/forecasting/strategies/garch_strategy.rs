...
impl GarchStrategy {
    ...
    fn forecast_volatility(&self, returns: &[f64]) -> Result<Vec<f64>> {
        ...
        if returns.len() < self.config.min_data_points {
            return Err(NyxsOwlError::MissingData(format!(
                ...
                returns.len(), self.config.min_data_points
            )));
        }
    ...
    fn garch_standard_enhanced(&self, returns: &[f64]) -> Result<Vec<f64>> {
        ...
        let (omega, alpha, beta) = self.estimate_garch_parameters_enhanced(returns, initial_variance)?;
    ...
    fn egarch_enhanced(&self, returns: &[f64]) -> Result<Vec<f64>> {
        ...
        for i in self.config.arch_order.max(1)..n {
            ...
            let innovation_term = alpha * (standardized_residual.abs() - (2.0 / std::f64::consts::PI).sqrt());
        ...
        let volatilities: Vec<f64> = log_volatilities.iter()
    ...
    fn gjr_garch_enhanced(&self, returns: &[f64]) -> Result<Vec<f64>> {
        ...
        let (omega, alpha, beta, gamma) = self.estimate_gjr_garch_parameters_enhanced(returns, initial_variance)?;
    ...
    fn estimate_garch_parameters_enhanced(&self, returns: &[f64], initial_variance: f64) -> Result<(f64, f64, f64)> {
    ...
    fn estimate_gjr_garch_parameters_enhanced(&self, returns: &[f64], initial_variance: f64) -> Result<(f64, f64, f64, f64)> {
        let sample_variance = returns.iter()
            .map(|&r| r.powi(2))
            .sum::<f64>() / returns.len() as f64;
    ...
    fn generate_volatility_breakout_signal(
        ...
        if vol_ratio > self.config.volatility_threshold {
            ...
            if recent_return.abs() > self.config.signal_threshold {
                ...
                return Ok(if recent_return > 0.0 { Signal::Buy } else { Signal::Sell });
            }
        }
        ...
        if vol_ratio > self.config.volatility_threshold * 1.5 {
            ...
            return Ok(if recent_return > 0.0 { Signal::Sell } else { Signal::Buy });
        }
...
mod tests {
    ...
    fn test_garch_strategy_config_validation() {
        // Valid configuration
        let config = GarchStrategyConfig::new(
            GarchType::Standard, 1, 1, 1.5, 0.02, 100
        );
        assert!(config.is_ok());

        // Invalid GARCH order
        let config = GarchStrategyConfig::new(
            GarchType::Standard, 0, 1, 1.5, 0.02, 100
        );
        assert!(config.is_err());

        // Invalid ARCH order
        let config = GarchStrategyConfig::new(
            GarchType::Standard, 1, 0, 1.5, 0.02, 100
        );
        assert!(config.is_err());

        // Invalid volatility threshold
        let config = GarchStrategyConfig::new(
            GarchType::Standard, 1, 1, -1.0, 0.02, 100
        );
        assert!(config.is_err());

        // Invalid signal threshold
        let config = GarchStrategyConfig::new(
            GarchType::Standard, 1, 1, 1.5, 1.5, 100
        );
        assert!(config.is_err());

        // Invalid min data points
        let config = GarchStrategyConfig::new(
            GarchType::Standard, 1, 1, 1.5, 0.02, 30
        );
        assert!(config.is_err());
    }
    ...
    fn test_volatility_breakout_strategy() {
        ...
        let prices: Vec<f64> = (0..120).enumerate().map(|(i, _)| {
            if i < 60 {
    ...
    fn test_rolling_average_volatility() {
        ...
        let avg_vol = strategy.calculate_rolling_average_volatility(&volatilities).unwrap();
