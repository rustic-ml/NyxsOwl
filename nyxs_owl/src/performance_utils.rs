...
impl SimdMath {
    ...
    pub unsafe fn add_f64_avx2(a: &[f64], b: &[f64], result: &mut [f64]) {
        ...
        assert!(a.len() % 4 == 0, "Array length must be multiple of 4 for AVX2");
...
impl SimdBatch {
    ...
    pub fn batch_correlations(series_data: &[&[f64]], lags: &[usize]) -> Vec<Vec<f64>> {
        series_data.iter().map(|series| {
            lags.iter().map(|&lag| {
                SimdMath::safe_autocorrelation(series, lag)
            }).collect()
        }).collect()
    }
...
impl SimdBenchmark {
    ...
    pub fn run_performance_comparison() {
        ...
        for size in sizes {
            ...
            let scalar_result = data1.iter().zip(data2.iter()).map(|(x, y)| x * y).sum::<f64>();
            ...
            println!("  • Dot Product:");
            println!("    - Scalar: {:?} (result: {:.6})", scalar_time, scalar_result);
            ...
            let scalar_variance = data1.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / (data1.len() - 1) as f64;
            ...
            println!("  • Variance:");
            println!("    - Scalar: {:?} (result: {:.6})", scalar_var_time, scalar_variance);
            println!("    - SIMD:   {:?} (result: {:.6})", simd_var_time, simd_variance);
