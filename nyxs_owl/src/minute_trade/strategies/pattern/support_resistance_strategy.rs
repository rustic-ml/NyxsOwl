...
impl SupportResistanceStrategy {
    ...
    fn identify_levels(&self, data: &[MinuteOhlcv], current_index: usize) -> Vec<PriceLevel> {
        ...
        for i in (start_index + 1)..(current_index - 1) {
            ...
            if curr.data.high > prev.data.high && curr.data.high > next.data.high {
                // Round to nearest 0.1% to group nearby levels
                let bucket = (curr.data.high * 1000.0) as i64;
                level_groups
                    .entry(bucket)
                    .and_modify(|(price, count, level_type, last_idx)| {
                        ...
                        *price = (*price * (*count - 1) as f64 + curr.data.high) / *count as f64;
                    })
                    .or_insert((curr.data.high, 1, LevelType::Resistance, i));
            }

            // Local low (potential support)
            if curr.data.low < prev.data.low && curr.data.low < next.data.low {
                // Round to nearest 0.1% to group nearby levels
                let bucket = (curr.data.low * 1000.0) as i64;
                level_groups
                    .entry(bucket)
                    .and_modify(|(price, count, level_type, last_idx)| {
                        *count += 1;
                        *last_idx = i;
                        // Calculate weighted average for level price
                        *price = (*price * (*count - 1) as f64 + curr.data.low) / *count as f64;
                    })
                    .or_insert((curr.data.low, 1, LevelType::Support, i));
            }
        }
    ...
    fn is_breakout(&self, candle: &MinuteOhlcv, level: &PriceLevel) -> bool {
        match level.level_type {
            LevelType::Support => {
                candle.data.close < level.price * (1.0 - self.level_zone_pct / 200.0)
            }
    ...
    fn is_bounce(
        &self,
        candle: &MinuteOhlcv,
        prev_candle: &MinuteOhlcv,
        level: &PriceLevel,
    ) -> bool {
        match level.level_type {
            LevelType::Support => {
                // Price approached support and bounced up
                prev_candle.data.low <= level.price * (1.0 + self.level_zone_pct / 100.0)
                    && candle.data.close > prev_candle.data.close
            }
            LevelType::Resistance => {
                // Price approached resistance and bounced down
                prev_candle.data.high >= level.price * (1.0 - self.level_zone_pct / 100.0)
                    && candle.data.close < prev_candle.data.close
            }
        }
    }
...
type OhlcvData = crate::minute_trade::MinuteOhlcv;
...
impl IntradayStrategy for SupportResistanceStrategy {
    ...
    fn generate_signals(&self, data: &[MinuteOhlcv]) -> Result<Vec<Signal>, TradeError> {
        ...
        for i in self.lookback_period..data.len() {
            ...
            if in_long || in_short {
                for level in &levels {
                    let is_near = self.is_near_level(current.data.close, level);
...
mod tests {
    ...
    fn test_level_detection() {
        ...
        let levels = strategy.identify_levels(&data, 100);

        // There should be at least some levels detected with our clear bouncing pattern
        assert!(!levels.is_empty(), "No levels detected with clear support/resistance pattern. Data length: {}, levels found: {}", data.len(), levels.len());
