use polars::prelude::{PolarsResult, Series, NamedFrom, DataType, IntoSeries, RollingOptionsFixedWindow, FillNullStrategy, SeriesOpsTime, Float64Chunked};
...
fn periods_since_extremum(
    ...
    if period == 0 {
        return Err(PolarsError::ComputeError("Period must be greater than 0.".into()));
    }
    if prices.dtype() != &DataType::Float64 {
        return Err(PolarsError::ComputeError("Input series for periods_since_extremum must be Float64.".into()));
    }
            ...
            if let Some(val) = prices_ca.get(window_idx) {
                match extremum_val_opt {
                    Some(current_extremum) => {
                        if (find_max && val > current_extremum) || (!find_max && val < current_extremum) {
                            extremum_val_opt = Some(val);
                            extremum_idx = j;
                        }
                    }
                    None => { // First non-null value in window
                        extremum_val_opt = Some(val);
                        extremum_idx = j;
                    }
                }
...
pub fn calculate_aroon(high_prices: &Series, low_prices: &Series, period: usize) -> PolarsResult<(Series, Series)> {
    if period == 0 {
        return Err(PolarsError::ComputeError("Aroon period must be greater than 0.".into()));
    }
    if high_prices.len() != low_prices.len() {
        return Err(PolarsError::ComputeError("High and Low price series must have the same length for Aroon.".into()));
    }
     if high_prices.dtype() != &DataType::Float64 || low_prices.dtype() != &DataType::Float64 {
        return Err(PolarsError::ComputeError("High/Low series for Aroon must be Float64.".into()));
    }
...
pub fn calculate_aroon_oscillator(high_prices: &Series, low_prices: &Series, period: usize) -> PolarsResult<Series> {
    if period == 0 {
        return Err(PolarsError::ComputeError("Aroon Oscillator period must be greater than 0.".into()));
    }
...
mod aroon_tests { 
    ...
    fn test_periods_since_extremum_basic() -> PolarsResult<()> {
        let s = create_test_series_aroon("price", vec![Some(1.0), Some(2.0), Some(3.0), Some(1.0), Some(4.0)]);
        let period = 3;
        ...
        assert_eq!(since_high.get(2).unwrap().try_extract::<f64>().unwrap(), 0.0);
        assert_eq!(since_high.get(3).unwrap().try_extract::<f64>().unwrap(), 1.0);
        assert_eq!(since_high.get(4).unwrap().try_extract::<f64>().unwrap(), 0.0);
    ...
    fn test_aroon_calculation() -> PolarsResult<()> {
        let highs = create_test_series_aroon("high", vec![Some(10.0), Some(12.0), Some(11.0), Some(10.0), Some(13.0)]);
        let lows  = create_test_series_aroon("low",  vec![Some(5.0),  Some(7.0),  Some(6.0),  Some(8.0),  Some(9.0)]);
        let period = 3;
    ...
    fn test_aroon_oscillator_calculation() -> PolarsResult<()> {
        let highs = create_test_series_aroon("high", vec![Some(10.0), Some(12.0), Some(11.0), Some(10.0), Some(13.0), Some(12.0)]);
        let lows  = create_test_series_aroon("low",  vec![Some(5.0),  Some(7.0),  Some(6.0),  Some(8.0),  Some(9.0), Some(10.0)]);
        let period = 3;
        ...
        assert!(!val_2.is_nan() && val_2 >= -100.0 && val_2 <= 100.0, "Index 2 oscillator out of range: {}", val_2);
        assert!(!val_3.is_nan() && val_3 >= -100.0 && val_3 <= 100.0, "Index 3 oscillator out of range: {}", val_3);
        assert!(!val_4.is_nan() && val_4 >= -100.0 && val_4 <= 100.0, "Index 4 oscillator out of range: {}", val_4);
        assert!(!val_5.is_nan() && val_5 >= -100.0 && val_5 <= 100.0, "Index 5 oscillator out of range: {}", val_5);
        Ok(())
    }
...
pub fn wilders_smoothing(series: &Series, period: usize) -> PolarsResult<Series> {
    if period == 0 {
        return Err(PolarsError::ComputeError("Wilder's smoothing period must be greater than 0.".into()));
    }
    if series.dtype() != &DataType::Float64 {
        return Err(PolarsError::ComputeError("Input series for Wilder's smoothing must be Float64.".into()));
    }
    let span = (2 * period).saturating_sub(1);
    if span == 0 { 
         return Err(PolarsError::ComputeError("Wilder's smoothing period results in invalid span for ewm_mean.".into()));
    }
...
pub fn calculate_true_range(high: &Series, low: &Series, close: &Series) -> PolarsResult<Series> {
    if ![high.dtype(), low.dtype(), close.dtype()].iter().all(|d| **d == DataType::Float64) {
        return Err(PolarsError::ComputeError("High, Low, Close series for TR must be Float64.".into()));
    }
    if !(high.len() == low.len() && low.len() == close.len()) {
        return Err(PolarsError::ComputeError("High, Low, Close series must have the same length.".into()));
    }
    if high.is_empty() {
    ...
    let h_minus_l_abs = Float64Chunked::new("h_minus_l_abs".into(), &h_minus_l_abs_values).into_series();
    let h_minus_pc_abs = Float64Chunked::new("h_minus_pc_abs".into(), &h_minus_pc_abs_values).into_series();
    let l_minus_pc_abs = Float64Chunked::new("l_minus_pc_abs".into(), &l_minus_pc_abs_values).into_series();
...
pub fn calculate_directional_movement_components(high: &Series, low: &Series) -> PolarsResult<(Series, Series)> {
    if ![high.dtype(), low.dtype()].iter().all(|d| **d == DataType::Float64) {
        return Err(PolarsError::ComputeError("High, Low series for DM must be Float64.".into()));
    }
    if high.len() != low.len() {
        return Err(PolarsError::ComputeError("High and Low series must have the same length.".into()));
    }
    if high.is_empty() {
        return Ok((
...
pub fn calculate_adx_di(
    ...
    if period == 0 {
        return Err(PolarsError::ComputeError("ADX/DI period must be greater than 0.".into()));
    }
...
pub fn calculate_adxr(adx_series: &Series, period: usize) -> PolarsResult<Series> {
    if period == 0 {
        return Err(PolarsError::ComputeError("ADXR period must be greater than 0.".into()));
    }
    if adx_series.dtype() != &DataType::Float64 {
        return Err(PolarsError::ComputeError("Input ADX series for ADXR must be Float64.".into()));
    }
    if adx_series.is_empty() {
        return Ok(Series::new_empty(adx_series.name().clone(), &DataType::Float64));
    }
    ...
    for i in 0..adx_ca.len() {
        ...
        match (current_adx_opt, prev_adx_opt) {
            (Some(current_adx), Some(prev_adx)) => adxr_values.push(Some((current_adx + prev_adx) / 2.0)),
...
#[cfg(test)]
mod adx_tests { // New test module for ADX/DI
    use super::*;
    use polars::prelude::AnyValue;
    ...
    fn test_wilders_smoothing_basic() -> PolarsResult<()> {
        let s = Series::new("data".into(), vec![Some(1.0), Some(2.0), Some(3.0), Some(4.0), Some(5.0)]);
    ...
    fn test_true_range_basic() -> PolarsResult<()> {
        let high = Series::new("H".into(), vec![Some(10.0), Some(12.0), Some(11.0), Some(15.0)]);
        let low  = Series::new("L".into(), vec![Some(8.0),  Some(10.0), Some(9.0),  Some(12.0)]);
        let close= Series::new("C".into(), vec![Some(9.0),  Some(11.0), Some(10.0), Some(14.0)]);
        let tr = calculate_true_range(&high, &low, &close)?;
    ...
    fn test_directional_movement_basic() -> PolarsResult<()> {
        let high = Series::new("H".into(), vec![Some(10.0), Some(12.0), Some(11.0), Some(12.0), Some(10.0)]);
        let low  = Series::new("L".into(), vec![Some(8.0),  Some(9.0),  Some(10.0), Some(9.0),  Some(8.0)]);
        ...
        assert_eq!(pdm_vec, vec![Some(0.0), Some(2.0), Some(0.0), Some(0.0), Some(0.0)]);
        assert_eq!(mdm_vec, vec![Some(0.0), Some(0.0), Some(0.0), Some(0.0), Some(1.0)]);
        Ok(())
    }
    ...
    fn test_adx_di_calculation_conceptual() -> PolarsResult<()> {
        ...
        let high = Series::new("high".into(), vec![Some(10.0), Some(12.0), Some(11.0), Some(13.0), Some(14.0), Some(15.0)]);
        let low = Series::new("low".into(), vec![Some(9.0), Some(10.0), Some(10.0), Some(12.0), Some(13.0), Some(12.0)]);
        let close = Series::new("close".into(), vec![Some(9.5), Some(11.0), Some(10.5), Some(12.5), Some(13.5), Some(14.0)]);
        ...
        assert!(is_null_or_nan(adx.get(0).unwrap()), "ADX[0] should be Null or NaN");
        assert!(is_null_or_nan(adx.get(1).unwrap()), "ADX[1] should be Null or NaN");
        assert!(is_null_or_nan(adx.get(2).unwrap()), "ADX[2] should be Null or NaN");
        assert!(is_null_or_nan(adx.get(3).unwrap()), "ADX[3] should be Null or NaN");
        assert!(is_null_or_nan(adx.get(4).unwrap()), "ADX[4] should be Null or NaN");
    ...
    fn test_adxr_calculation() -> PolarsResult<()> {
        let adx_values = vec![None, None, Some(10.0), Some(15.0), Some(20.0), Some(25.0), Some(30.0)];
        ...
        assert_eq!(adxr.get(4).unwrap().try_extract::<f64>().unwrap(), 15.0, "ADXR[4]"); // (20+10)/2
        assert_eq!(adxr.get(5).unwrap().try_extract::<f64>().unwrap(), 20.0, "ADXR[5]"); // (25+15)/2
        assert_eq!(adxr.get(6).unwrap().try_extract::<f64>().unwrap(), 25.0, "ADXR[6]"); // (30+20)/2
...
fn calculate_hl_avg(high_series: &Series, low_series: &Series, period: usize) -> PolarsResult<Series> {
    if period == 0 {
        return Err(PolarsError::ComputeError("Period for HL_avg must be > 0.".into()));
    }
    if high_series.len() != low_series.len() {
        return Err(PolarsError::ComputeError("High and Low series must have the same length.".into()));
    }
    if high_series.dtype() != &DataType::Float64 || low_series.dtype() != &DataType::Float64 {
        return Err(PolarsError::ComputeError("High/Low series for HL_avg must be Float64.".into()));
    }
...
pub fn calculate_ichimoku_cloud(
    ...
    if tenkan_period == 0 || kijun_period == 0 || senkou_span_b_period == 0 {
        return Err(PolarsError::ComputeError("Ichimoku periods must be greater than 0.".into()));
    }
    ...
    Ok((tenkan_sen, kijun_sen, senkou_span_a, senkou_span_b, chikou_span))
}
...
mod ichimoku_tests { 
    ...
    fn create_test_data_ichimoku(len: usize) -> (Series, Series, Series) {
        ...
        (Series::new("high".into(), highs), Series::new("low".into(), lows), Series::new("close".into(), closes))
    }

    #[test]
    fn test_hl_avg_basic() -> PolarsResult<()> {
        let highs = Series::new("h".into(), vec![Some(10.0), Some(12.0), Some(11.0), Some(13.0), Some(15.0)]);
        let lows  = Series::new("l".into(), vec![Some(8.0),  Some(9.0),  Some(10.0), Some(10.0), Some(12.0)]);
        let period = 3;
    ...
    fn test_ichimoku_cloud_calculation_runs() -> PolarsResult<()> {
        ...
        let result = calculate_ichimoku_cloud(&highs, &lows, &closes, tenkan_p, kijun_p, senkou_b_p, lag_p, lead_p);
        assert!(result.is_ok());
        ...
        assert!(non_null_span_a, "Senkou Span A should have non-null values after lead period");
        ...
        assert!(non_null_chikou, "Chikou Span should have non-null values before lag period ends");
    ...
    fn test_ichimoku_invalid_periods() {
        ...
        assert!(calculate_ichimoku_cloud(&h, &l, &c, 9, 26, 0, 26, 26).is_err()); // senkou_b_period = 0
    }
}
...
pub fn calculate_psar(high_prices: &Series, low_prices: &Series, initial_af: f64, max_af: f64) -> PolarsResult<Series> {
    ...
    if high_prices.len() != low_prices.len() {
        return Err(PolarsError::ComputeError("High and Low price series must have the same length for PSAR.".into()));
    }
    if high_prices.dtype() != &DataType::Float64 || low_prices.dtype() != &DataType::Float64 {
        return Err(PolarsError::ComputeError("High/Low series for PSAR must be Float64.".into()));
    }

    let len = high_prices.len();
    if len < 2 { // Need at least two points to determine initial trend and calculate first SAR
    ...
    if let (Some(h1), Some(l1), Some(h2), Some(l2)) = (high_ca.get(0), low_ca.get(0), high_ca.get(1), low_ca.get(1)) {
        if h2 > h1 { // Tentative rising trend if second high is higher
        ...
        } else if l2 < l1 { // Tentative falling trend
        ...
        } else { // Indecisive or equal, default to using the first low as SAR, assume uptrend for a start
            is_rising_trend = true;
            current_sar = l1;
            extreme_point = h1;
        }
    ...
    if let (Some(h0), Some(l0)) = (high_ca.get(0), low_ca.get(0)) {
        ...
        if h1 > h0 && l1 > l0 { // Initial trend assumed up if 2nd bar is higher high and higher low
            is_rising_trend = true;
            current_sar = l0; 
            extreme_point = h1;
        } else if l1 < l0 && h1 < h0 { // Initial trend assumed down
            is_rising_trend = false;
            current_sar = h0;
            extreme_point = l1;
        } else { // Mixed or inside bar, default to prior low, assume uptrend
            is_rising_trend = true;
            current_sar = l0;
            extreme_point = h1;
        }
    ...
    for i in 2..len {
        ...
        if is_rising_trend {
            ...
            if current_low < current_sar { // Trend reverses to falling
                is_rising_trend = false;
            ...
            if current_high > current_sar { // Trend reverses to rising
                is_rising_trend = true;
...
mod psar_tests {
    ...
    fn test_psar_basic_rising_trend() -> PolarsResult<()> {
        let highs = create_test_series_psar("high", vec![Some(10.0), Some(11.0), Some(12.0), Some(13.0), Some(14.0)]);
        let lows  = create_test_series_psar("low",  vec![Some(9.0),  Some(10.0), Some(11.0), Some(12.0), Some(13.0)]);
        let initial_af = 0.02;
        let max_af = 0.20;
    ...
    fn test_psar_basic_falling_trend() -> PolarsResult<()> {
        let highs = create_test_series_psar("high", vec![Some(14.0), Some(13.0), Some(12.0), Some(11.0), Some(10.0)]);
        let lows  = create_test_series_psar("low",  vec![Some(13.0), Some(12.0), Some(11.0), Some(10.0), Some(9.0)]);
        let initial_af = 0.02;
        let max_af = 0.20;
        let psar = calculate_psar(&highs, &lows, initial_af, max_af)?;
    ...
    fn test_psar_reversal() -> PolarsResult<()> {
        let highs = create_test_series_psar("high", vec![Some(10.0), Some(11.0), Some(10.5), Some(9.0), Some(8.0)]);
        let lows  = create_test_series_psar("low",  vec![Some(9.5),  Some(10.2), Some(9.0),  Some(8.5), Some(7.0)]);
        let initial_af = 0.02;
        let max_af = 0.20;
        ...
        assert_eq!(psar.get(0).unwrap(), AnyValue::Null);
        assert_eq!(psar.get(1).unwrap().try_extract::<f64>().unwrap(), 9.5, "PSAR[1]");
        assert_eq!(psar.get(2).unwrap().try_extract::<f64>().unwrap(), 11.0, "PSAR[2] Reversal");
        assert_eq!(psar.get(3).unwrap().try_extract::<f64>().unwrap(), 10.5, "PSAR[3]");
        assert_eq!(psar.get(4).unwrap().try_extract::<f64>().unwrap(), 9.0, "PSAR[4]");
    ...
    fn test_psar_with_nones_in_data() -> PolarsResult<()> {
        let highs = create_test_series_psar("high", vec![Some(10.0), Some(11.0), None, Some(13.0), Some(14.0)]);
        let lows  = create_test_series_psar("low",  vec![Some(9.0),  Some(10.0), Some(11.0), Some(12.0), Some(13.0)]);
        let psar = calculate_psar(&highs, &lows, 0.02, 0.2)?;
...
pub fn calculate_vortex(
    ...
    if period == 0 {
        return Err(PolarsError::ComputeError("Vortex Indicator period must be greater than 0.".into()));
    }
    if !(high_prices.len() == low_prices.len() && low_prices.len() == close_prices.len()) {
        return Err(PolarsError::ComputeError("High, Low, and Close series must have the same length for Vortex.".into()));
    }
    if ![high_prices.dtype(), low_prices.dtype(), close_prices.dtype()].iter().all(|&d| d == &DataType::Float64) {
        return Err(PolarsError::ComputeError("Input series for Vortex must be Float64.".into()));
    }
    if high_prices.len() < period {
    ...
    let vi_plus_cleaned = Float64Chunked::new(format!("VI_Plus_{}", period).into(), &vi_plus_cleaned_values).into_series();
    ...
    let vi_minus_cleaned = Float64Chunked::new(format!("VI_Minus_{}", period).into(), &vi_minus_cleaned_values).into_series();
...
mod vortex_tests {
    ...
    fn test_vortex_calculation_basic() -> PolarsResult<()> {
        let highs = create_test_series_vortex("high", vec![Some(10.0), Some(12.0), Some(11.0), Some(13.0), Some(14.0)]);
        let lows  = create_test_series_vortex("low",  vec![Some(8.0),  Some(10.0), Some(9.0),  Some(12.0), Some(13.0)]);
        let closes= create_test_series_vortex("close",vec![Some(9.0),  Some(11.0), Some(10.0), Some(12.5), Some(13.5)]);
        let period = 3;
        ...
        assert!((vi_p.get(3).unwrap().try_extract::<f64>().unwrap() - 1.125).abs() < 0.0001, "VI+ at index 3");
        
        // VI- at index 3 should be 0.5 based on manual calculation above
        assert!((vi_m.get(3).unwrap().try_extract::<f64>().unwrap() - 0.5).abs() < 0.0001, "VI- at index 3");
        ...
        assert!(vi_minus_4.is_ok(), "VI- at index 4 should be a valid number");
    ...
    fn test_vortex_division_by_zero_in_sum_tr() -> PolarsResult<()> {
        ...
        let highs  = create_test_series_vortex("h", vec![Some(10.0), Some(10.0), Some(10.0), Some(10.0), Some(11.0)]);
        let lows   = create_test_series_vortex("l", vec![Some(10.0), Some(10.0), Some(10.0), Some(10.0), Some(10.5)]);
        let closes = create_test_series_vortex("c", vec![Some(10.0), Some(10.0), Some(10.0), Some(10.0), Some(10.8)]);
        let period = 3;
        ...
        assert_eq!(vi_p.get(0).unwrap(), AnyValue::Null); 
        assert_eq!(vi_p.get(1).unwrap(), AnyValue::Null); 
        assert_eq!(vi_p.get(2).unwrap(), AnyValue::Null, "Expected Null for VI+ at index 2 due to SumTR=0");
        
        // Check index 3, window indices 1,2,3 (all 10s)
        // SumTR = 0+0+0=0
        assert_eq!(vi_p.get(3).unwrap(), AnyValue::Null, "Expected Null for VI+ at index 3 due to SumTR=0");
