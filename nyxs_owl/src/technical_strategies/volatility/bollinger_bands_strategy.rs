...
pub fn bollinger_bands_signals(
    ...
    for i in first_valid_signal_idx..data_len {
        ...
        if let (Some(current_close), Some(prev_close), Some(current_upper), Some(current_lower), Some(prev_upper), Some(prev_lower)) =
            (current_close_opt, prev_close_opt, current_upper_opt, current_lower_opt, prev_upper_opt, prev_lower_opt)
        {
    ...
    fn test_bollinger_bands_signals_conceptual() {
        ...
        match bollinger_bands_signals(&df, "close", period, std_dev) {
            Ok(signals) => {
                ...
                if df.height() > period + 5 { // Ensure enough data for signals to form
