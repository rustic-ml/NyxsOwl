...
use crate::technical_strategies::{TechnicalSignal, TechnicalStrategy, PerformanceMetrics};
...
impl ATRStrategy {
    ...
    fn calculate_volatility_ratio(&self, atr_values: &[f64], index: usize) -> f64 {
        ...
        let avg_atr: f64 = atr_values[start_idx..index]
            .iter()
            .filter(|&&x| x > 0.0)
            .sum::<f64>() / (index - start_idx) as f64;
    ...
    fn generate_signals(&self, data: &DataFrame, atr_values: &[f64]) -> NyxsOwlResult<Vec<TechnicalSignal>> {
        ...
        for i in 0..atr_values.len() {
            ...
            let (signal, strength, confidence) = if volatility_ratio >= self.config.expansion_threshold {
...
impl TechnicalStrategy for ATRStrategy {
    ...
    fn validate_parameters(&self) -> NyxsOwlResult<()> {
        if self.config.period == 0 {
            return Err(NyxsOwlError::ValidationError("Period must be greater than 0".to_string()));
        }
        
        if self.config.expansion_threshold <= self.config.contraction_threshold {
            return Err(NyxsOwlError::ValidationError("Expansion threshold must be greater than contraction threshold".to_string()));
        }
        
        if self.config.stop_loss_multiplier <= 0.0 {
            return Err(NyxsOwlError::ValidationError("Stop loss multiplier must be positive".to_string()));
        }
        
        if self.config.position_size_multiplier <= 0.0 || self.config.position_size_multiplier > 0.1 {
            return Err(NyxsOwlError::ValidationError("Position size multiplier must be between 0 and 0.1".to_string()));
        }
...
mod tests {
    ...
    fn create_test_data() -> DataFrame {
        ...
        let highs: Vec<f64> = (0..50).map(|i| 100.0 + (i as f64 * 0.5) + ((i % 5) as f64) * 2.0).collect();
        let lows: Vec<f64> = highs.iter().map(|&h| h - 3.0 - ((h as usize % 4) as f64)).collect();
        let closes: Vec<f64> = highs.iter().zip(lows.iter()).map(|(&h, &l)| l + (h - l) * 0.7).collect();
    ...
    fn test_position_sizing() {
        ...
        assert!(position_size_high_atr < position_size, "Higher ATR should yield smaller position size");
    }
    ...
    fn test_stop_loss_calculation() {
        ...
        assert!(long_stop < 100.0, "Long stop loss should be below entry price");
        assert!(short_stop > 100.0, "Short stop loss should be above entry price");
    }
