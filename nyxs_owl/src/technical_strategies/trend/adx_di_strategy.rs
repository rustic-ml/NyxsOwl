...
pub fn adx_di_signals(
    ...
    if period == 0 {
        return Err(NyxsOwlError::InvalidParameter("ADX/DI period must be greater than 0.".into()));
    }

    let high_series = df.column(high_column).map_err(|e| NyxsOwlError::DataError(format!("High column '{}' not found: {}", high_column, e)))?;
    let low_series = df.column(low_column).map_err(|e| NyxsOwlError::DataError(format!("Low column '{}' not found: {}", low_column, e)))?;
    let close_series = df.column(close_column).map_err(|e| NyxsOwlError::DataError(format!("Close column '{}' not found: {}", close_column, e)))?;

    let data_len = df.height();
    // calculate_adx_di itself checks for sufficient length (period*2)
    if data_len < period * 2 { // Basic check, more robust in calculate_adx_di
    ...
    let (adx_series, plus_di_series, minus_di_series) = 
        calculate_adx_di(high_series, low_series, close_series, period).map_err(|e| {
            NyxsOwlError::StrategyError(format!("Failed to calculate ADX/DI: {}", e))
        })?;

    let adx_ca = adx_series.f64().map_err(|_| NyxsOwlError::StrategyError("ADX Series is not Float64".into()))?;
    let plus_di_ca = plus_di_series.f64().map_err(|_| NyxsOwlError::StrategyError("+DI Series is not Float64".into()))?;
    let minus_di_ca = minus_di_series.f64().map_err(|_| NyxsOwlError::StrategyError("-DI Series is not Float64".into()))?;
    ...
    for i in first_signal_idx..data_len {
        ...
        if let (Some(adx_val), Some(current_plus_di), Some(current_minus_di), Some(prev_plus_di), Some(prev_minus_di)) = 
            (adx_val_opt, current_plus_di_opt, current_minus_di_opt, prev_plus_di_opt, prev_minus_di_opt)
        {
...
mod tests {
    ...
    fn test_adx_di_invalid_period() {
        ...
        assert!(matches!(adx_di_signals(&df, "high", "low", "close", 0, 20.0), Err(NyxsOwlError::InvalidParameter(_))));
    }
    ...
    fn test_adx_di_insufficient_data() {
        ...
        assert!(matches!(adx_di_signals(&df_too_short, "high", "low", "close", period, 20.0), Err(NyxsOwlError::MissingData(_))));
    ...
    fn test_adx_di_columns_not_found() {
        let df = create_test_df_adx(50).unwrap();
        assert!(matches!(adx_di_signals(&df, "non_existent", "low", "close", 14, 20.0), Err(NyxsOwlError::DataError(_))));
        assert!(matches!(adx_di_signals(&df, "high", "non_existent", "close", 14, 20.0), Err(NyxsOwlError::DataError(_))));
        assert!(matches!(adx_di_signals(&df, "high", "low", "non_existent", 14, 20.0), Err(NyxsOwlError::DataError(_))));
    }
    ...
    fn test_adx_di_signals_conceptual() {
        ...
        match adx_di_signals(&df, "high", "low", "close", period, adx_threshold) {
            Ok(signals) => {
                ...
                assert!(has_buy_signal || has_sell_signal, "Expected ADX/DI strategy to generate some signals with varied data.");
            },
