...
pub fn macd_signals(
    ...
    let macd_values: Vec<Option<f64>> = macd_line.f64()
        .map_err(|e| NyxsOwlError::DataError(format!("Failed to extract MACD values: {}", e)))?
        .into_iter().collect();

    let signal_values: Vec<Option<f64>> = signal_line.f64()
    ...
    let mut previous_macd_above_signal = None;

    for i in 1..macd_values.len() {
        if let (Some(current_macd), Some(current_signal), Some(prev_macd), Some(prev_signal)) = 
           (macd_values[i], signal_values[i], macd_values.get(i-1).and_then(|&x| x), signal_values.get(i-1).and_then(|&x| x)) {
...
mod tests {
    ...
    fn create_test_df_for_macd_strategy(len: usize) -> PolarsResult<DataFrame> {
        let prices: Vec<f64> = (0..len).map(|i| {
            50.0 + 10.0 * ( (i as f64 * 0.1).sin() ) + (i as f64 * 0.05)
        }).collect(); 
        df! {
            "close" => prices
        }
    ...
    fn test_macd_strategy_invalid_periods() {
        ...
        // Zero periods
        assert!(matches!(macd_signals(&df, "close", 0, 26, 9), Err(NyxsOwlError::InvalidParameter(_))));
        assert!(matches!(macd_signals(&df, "close", 12, 0, 9), Err(NyxsOwlError::InvalidParameter(_))));
        assert!(matches!(macd_signals(&df, "close", 12, 26, 0), Err(NyxsOwlError::InvalidParameter(_))));
        
        // Fast >= slow
        assert!(matches!(macd_signals(&df, "close", 26, 12, 9), Err(NyxsOwlError::InvalidParameter(_))));
        assert!(matches!(macd_signals(&df, "close", 26, 26, 9), Err(NyxsOwlError::InvalidParameter(_))));
    }
    ...
    fn test_macd_strategy_insufficient_data() {
        ...
        assert!(matches!(macd_signals(&df_too_short, "close", fast, slow, signal_p), Err(NyxsOwlError::MissingData(_))));

        let df_just_enough_for_strategy = create_test_df_for_macd_strategy(required_len_for_strategy).unwrap();
    ...
    fn test_macd_strategy_signals_generation() {
        ...
        // With oscillating price data, should have some buy and sell signals
        let buy_count = signals.iter().filter(|&&s| s == Signal::Buy).count();
        let sell_count = signals.iter().filter(|&&s| s == Signal::Sell).count();
        
        // Should have at least some trading signals
        assert!(buy_count + sell_count > 0, "Expected MACD strategy to generate some trading signals");
    }
} 