...
use nyxs_owl::minute_trade::{
    create_test_data, IntradayStrategy, Signal,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sell_signals = volume_signals.iter().filter(|&&s| s == Signal::Sell).count();
    let hold_signals = volume_signals.iter().filter(|&&s| s == Signal::Hold).count();
    ...
    let session_performance = session_strategy.calculate_performance(&test_data, &session_signals)?;
    
    let buy_signals = session_signals.iter().filter(|&&s| s == Signal::Buy).count();
    let sell_signals = session_signals.iter().filter(|&&s| s == Signal::Sell).count();
    let hold_signals = session_signals.iter().filter(|&&s| s == Signal::Hold).count();
    ...
    let regression_performance = regression_strategy.calculate_performance(&test_data, &regression_signals)?;
    
    let buy_signals = regression_signals.iter().filter(|&&s| s == Signal::Buy).count();
    let sell_signals = regression_signals.iter().filter(|&&s| s == Signal::Sell).count();
    let hold_signals = regression_signals.iter().filter(|&&s| s == Signal::Hold).count();
    ...
    let best_strategy = strategies.iter().max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap());
    ...
    for (name, signals) in all_signals {
        let buy_pct = signals.iter().filter(|&&s| s == Signal::Buy).count() as f64 / signals.len() as f64 * 100.0;
        let sell_pct = signals.iter().filter(|&&s| s == Signal::Sell).count() as f64 / signals.len() as f64 * 100.0;
        let hold_pct = signals.iter().filter(|&&s| s == Signal::Hold).count() as f64 / signals.len() as f64 * 100.0;
    ...
    println!("✅ All {} strategies are fully functional and generating signals", strategies.len());
