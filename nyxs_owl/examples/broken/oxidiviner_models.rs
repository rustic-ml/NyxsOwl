...
use forecast_trade::models::oxidiviner::{ExponentialSmoothingAdapter, MovingAverageAdapter};
...
use forecast_trade::ForecastModel;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let daily_es = ExponentialSmoothingAdapter::new(0.2)?;
    let minute_es =
        ExponentialSmoothingAdapter::new(0.4)?.with_granularity(TimeGranularity::Minute);
    ...
    // Create models with different granularities
    let daily_ma = MovingAverageAdapter::new(20)?;
    let minute_ma = MovingAverageAdapter::new(60)?.with_granularity(TimeGranularity::Minute);
    ...
    let minute_mr_strategy =
        MeanReversionStrategy::new_with_granularity(minute_es, 1.5, TimeGranularity::Minute)?;
    ...
    let daily_tf_strategy = TrendFollowingStrategy::new(daily_ma, 20)?;
    let minute_tf_strategy =
        TrendFollowingStrategy::new_with_granularity(minute_ma, 60, TimeGranularity::Minute)?;
    ...
    let daily_vb_strategy =
        VolatilityBreakoutStrategy::new(ExponentialSmoothingAdapter::new(0.2)?, 1.5)?;
    let minute_vb_strategy = VolatilityBreakoutStrategy::new_with_granularity(
        ExponentialSmoothingAdapter::new(0.4)?,
        2.0,
