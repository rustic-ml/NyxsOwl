...
use forecast_trade::strategies::{BacktestResult, ForecastStrategy, TimeGranularity};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let daily_model = ExponentialSmoothingAdapter::new(0.7)?;
    let minute_model =
        ExponentialSmoothingAdapter::new(0.3)?.with_granularity(TimeGranularity::Minute);
...
fn print_backtest_results(results: &BacktestResult) {
