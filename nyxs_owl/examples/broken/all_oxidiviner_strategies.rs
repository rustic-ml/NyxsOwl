...
use forecast_trade::models::oxidiviner::{
    Autoregressive, ExponentialSmoothing, Garch, MovingAverage,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let minute_tf_strategy =
        TrendFollowingStrategy::new_with_granularity(minute_ma, 0.2, TimeGranularity::Minute)?;
    ...
    let minute_arima_strategy =
        ArimaStrategy::new_with_granularity(minute_arima, 0.5, TimeGranularity::Minute)?;
    ...
    let minute_vol_strategy =
        VolatilityStrategy::new_with_granularity(minute_garch, 1.2, TimeGranularity::Minute)?;
