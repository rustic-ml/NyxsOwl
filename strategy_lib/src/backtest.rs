    ...
    for &col in strategy.required_columns().iter() {
        ...
        if data.column(col).is_err() {
            return Err(StrategyError::MissingData(
                format!("Required column '{}' not found in data", col)
            ));
        }
    }
...
mod tests {
    use super::*;
    use crate::strategy::{StrategyConfig, Signal};
    ...
    impl Strategy for MockStrategy {
        fn new(_config: StrategyConfig) -> Self {
            Self {
                ...
                signals: Series::new("signal".into(), vec![Signal::Hold as i32, Signal::Buy as i32, Signal::Sell as i32]),
            }
        }
