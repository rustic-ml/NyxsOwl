...
use crate::strategy::{Strategy, StrategyConfig, Signal, StrategyError};
...
impl Strategy for MovingAverageCrossover {
    fn new(_config: StrategyConfig) -> Self {
        ...
        let name = format!("MA_Crossover_{}_{}", 
    ...
    fn generate_signals(&self, data: &DataFrame) -> Result<Series, StrategyError> {
        ...
        if data.column(&self.config.price_col).is_err() {
            return Err(StrategyError::MissingData(
                format!("Price column '{}' not found in data", self.config.price_col)
            ));
        }
...
mod tests {
    ...
    fn test_strategy_creation() {
        ...
        assert_eq!(strategy.description(), "Moving Average Crossover strategy using 10 and 30 period EMA");
