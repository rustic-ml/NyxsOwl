...
use polars::prelude::*;
use crate::strategy::{Signal, StrategyError};
...
pub fn calculate_crossovers(series1: &Series, series2: &Series) -> Result<Series, StrategyError> {
    ...
    let s1 = series1.cast(&DataType::Float64).map_err(|e| StrategyError::PolarsError(e))?;
    let s2 = series2.cast(&DataType::Float64).map_err(|e| StrategyError::PolarsError(e))?;
...
pub fn calculate_pct_change(series: &Series) -> Result<Series, StrategyError> {
    ...
    let s = series.cast(&DataType::Float64).map_err(|e| StrategyError::PolarsError(e))?;
    ...
    Series::new(name.into(), pct_changes).cast(&DataType::Float64).map_err(|e| StrategyError::PolarsError(e))
}
...
mod tests {
    ...
    fn test_raw_values_to_signals() {
        let raw_signals = Series::new("raw", &[1, 0, -1, 1, 0, -1]);
    ...
    fn test_raw_values_to_signals_invalid_type() {
        let raw_signals = Series::new("raw", &["buy", "hold", "sell"]);
    ...
    fn test_calculate_crossovers() {
        ...
        let s1 = Series::new("s1", &[10.0, 11.0, 9.0, 8.0, 12.0, 14.0]);
        let s2 = Series::new("s2", &[11.0, 10.0, 10.0, 9.0, 10.0, 15.0]);
    ...
    fn test_calculate_crossovers_invalid_type() {
        let s1 = Series::new("s1", &[10.0, 11.0, 9.0]);
        let s2 = Series::new("s2", &["a", "b", "c"]);
    ...
    fn test_calculate_pct_change() {
        let values = Series::new("values", &[100.0, 110.0, 99.0, 105.0]);
        ...
        assert_relative_eq!(pct_vals.get(3).unwrap(), 0.06060606060606055, epsilon = 1e-10);
    }
    
    #[test]
    fn test_calculate_pct_change_with_zero() {
        let values = Series::new("values", &[0.0, 10.0, 0.0, 5.0]);
        let pct_change = calculate_pct_change(&values).unwrap();
    ...
    fn test_calculate_pct_change_invalid_type() {
        let values = Series::new("values", &["a", "b", "c"]);
